...
- **<abbr title="A contrastive loss treating the matching pair as the correct class in a softmax over the anchor's similarities to every other batch item.">InfoNCE</abbr> / NT-Xent** (symmetric): For <abbr title="The item whose embedding is being pulled toward its positive and pushed away from its negatives.">anchor</abbr> i with positive j, loss = −log( exp(sim(z_i, z_j)/τ) / Σ_{k≠i} exp(sim(z_i, z_k)/τ) ). <abbr title="A scalar dividing the similarities. Low values concentrate the gradient on the hardest negatives; high values spread it evenly.">Temperature τ</abbr> controls sharpness: low τ makes the loss focus on <abbr title="Non-matching items that the model currently scores as very similar to the anchor — the most informative ones to push apart.">hard negatives</abbr>, high τ spreads the gradient across all negatives.
