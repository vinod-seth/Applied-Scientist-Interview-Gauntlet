# Lesson 3 — Depth Drills & Mock Round: Systems Bar Raiser
...
> 📍 **Roadmap:** Depth drills first (self-score honestly), then the mock round. Session 1's assessment tested whether you understood what you *built*. This one tests whether you can defend what you *concluded* — a harder standard, because conclusions have confounds and instruments have error bars.

## 🟢 Before You Start

1. Open [PROGRESS.md](../../PROGRESS.md) in a second window.
2. Every Session 2 Metric Vault slot must be filled from run logs or explicitly marked `QUALITATIVE-ONLY`. The mock round probes what you filled.
...
Attempt all three levels per topic, in order. Record your honest level. Stop when your depth runs out and log it — that's the map working.

### Topic 1: Failure-Mode Operational Definitions

**Level 1 — State it:** Name the three failure modes and state, in one sentence each, the instrumented test that identifies them.

**Level 2 — Derive it:** Write the classifier as an ordered decision tree, including what labels it requires. Then name the two boundary cases where the buckets overlap.
...
**Level 1 — State it:** What did you sweep, over what range, and what was held fixed?

**Level 2 — Derive it:** Explain the recall-up/error-flat dissociation and what it licenses you to conclude. Then describe the oracle-context experiment and what it isolates.
...
**Level 1 — State it:** Name two mechanisms by which chunk size changes answer quality.

**Level 2 — Derive it:** Give all three (dilution, fragmentation, position) and explain how chunk size and k interact through the context budget.
...
1. Two competing effects, and saying so is the answer: dilution rises (each embedding averages more topics → retrieval-miss grows), while fragmentation falls (more evidence per chunk). Net direction depends on whether your answers span sentence boundaries. Also, 10×512 tokens of context invites position effects. A confident single-direction answer without the tension is a Level 2 answer at best.
...
5. Coarse grids hide non-monotonicity — the optimum may sit between your points, and a two-point "trend" can be noise. Naming the interaction (chunk×k both feed context length, so grid corners are not comparable conditions) is the detail that wins the exchange.
</details>
...
**Level 1 — State it:** Name your judge and what it decides.

**Level 2 — Derive it:** State each judge type's bias direction and which failure bucket that bias inflates or deflates.
...
**Level 1 — State it:** Define calibration and write the ECE formula with your configuration.

**Level 2 — Derive it:** Name three ways your ECE number lies, and what you'd report alongside it.
...
**Level 1 — State it:** What is TS, and what's your fitted T?

**Level 2 — Derive it:** Prove accuracy invariance; justify NLL as the objective; explain why one parameter suffices in-distribution.
...
**Level 1 — State it:** State the phenomenon: what happens to accuracy and confidence as severity rises?

**Level 2 — Derive it:** Give the mechanism — off-manifold features, linear head extrapolation, softmax saturation — and why it's no longer a uniform inflation.
...
3. It helps ECE at each severity, but severity isn't observable in production and corruption *type* also matters — so it collapses into shift detection plus a lookup, which is a different (harder) system. Recognizing it as shift detection in disguise is the strongest move.
...
**Level 1 — State it:** Name two things you'd build for a team deploying under expected shift.

**Level 2 — Derive it:** Describe selective prediction and the risk–coverage curve; explain what shift detection adds that calibration doesn't.
...
Update PROGRESS.md: mark each of the 8 topics `[k]`/`[d]`/`[D]`, log every gap with the exact chain and depth reached. Aim for Level 3 on all 8 before the mock round. Not there yet? Study the gap log, re-read the refresher at the exact stall point, and re-drill — nothing is gated on it.

---

## 🟢 Mock Round: Systems Bar Raiser
...
*(Expecting: the ordered decision tree — gold-in-context by a stated overlap rule, then answer-support against context, then support against any retrieved chunk. Plus bucket counts `[FILL]` and the taxonomy's own error rate. Naming boundary cases unprompted is the signal they're looking for.)*
...
**Passing condition:** all 10 at Bar-clearing answer. Zero hand-waves, zero unsupported numbers, zero overclaims.

### After the Mock Round

1. **Update PROGRESS.md:** tier each of the 8 core topics; log each drill with depth survived and where you run out of depth; record gaps with exact chain and depth.
2. **If it went well:** update the Consistency Tracker and say **next** when you're ready for Session 3.
3. **If not passed:** study the gap log, re-read the exact refresher where you stalled, re-drill that topic's Level 3 chain, retry from question one.
...
- Passing is followed by Session 3: ML Fundamentals — Core Theory.
