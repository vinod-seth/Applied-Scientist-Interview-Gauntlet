...
**Definition.** A classifier is calibrated if, among predictions made with confidence p, the accuracy is p. ECE estimates the expected gap: partition predictions into M confidence bins B₁…B_M and compute
...
2. **Mass concentration.** Modern networks pile predictions into the top bins (confidence 0.9–1.0); most fixed-width bins are nearly empty, so the estimate rides on two or three bins. Adaptive (equal-mass) binning is the standard fix — did you check both? `[FILL]`.
...
**Reliability diagrams** are the visual: per-bin accuracy against per-bin confidence; the identity line is perfect calibration; bars sagging below the line = overconfidence. Read them with the bin-count histogram attached — a dramatic gap in a bin holding 0.1% of the mass is noise, not signal.
...
Post-hoc calibration: keep the model frozen, learn a single scalar T > 0, and replace softmax(z) with softmax(z/T). Fit T by minimizing NLL on a held-out **in-distribution** validation set. Three facts you must produce on demand:

1. **Why accuracy is invariant:** z/T is a strictly monotone transform of the logits per example, so argmax never changes. Only the confidence *distribution* moves — T > 1 flattens (fixes overconfidence), T < 1 sharpens.
...
**The mechanism — this is the Gold-tier answer.** Temperature scaling fits one scalar to the *clean* validation distribution's logit geometry. Under covariate shift, inputs move off the training manifold; feature activations land in regions the classifier head still maps to large-margin logits (the head is a linear map — it extrapolates confidently by construction; softmax then saturates). The miscalibration is no longer a uniform logit inflation — it grows with severity and varies by corruption type — so no single T fitted on clean data can be right for the whole family of shifted distributions. The clean diagnostic: fit an **oracle temperature per severity** — you'll find T*(severity) increases monotonically, which is the direct exhibit that one scalar cannot serve all severities. Did you run it? `[FILL: oracle-T per severity, or name it as the missing experiment]`.

**What actually helps under shift** (know the landscape, one line each): deep ensembles remain the strongest simple baseline for shift-robust uncertainty (Lakshminarayanan et al. 2017, *Simple and Scalable Predictive Uncertainty Estimation using Deep Ensembles*, https://arxiv.org/abs/1612.01474 — and the best performer in Ovadia et al.'s study); MC dropout is cheaper and weaker; selective prediction/abstention converts bad calibration into controlled coverage; shift detection (feature-space distance, energy scores) flags when confidence shouldn't be trusted at all.
