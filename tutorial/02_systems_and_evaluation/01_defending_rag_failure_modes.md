...
"Retrieval-miss vs. retrieved-but-ignored vs. hallucination" sounds clean until you write the classifier. The operational definitions require **gold-passage labels** — for each question, which passage(s) contain the answer:
...
**Leakage.** The generator's pretraining may contain your QA corpus's facts. A closed-book baseline (no retrieval at all) measures this: if closed-book accuracy is already high, "retrieval-miss" cases can still be answered right and your taxonomy's denominators shift. Closed-book number: `[FILL or QUALITATIVE-ONLY]`.
